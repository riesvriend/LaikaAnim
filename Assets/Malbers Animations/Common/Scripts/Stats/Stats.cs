...
namespace MalbersAnimations
{
    ...
    public class Stats : MonoBehaviour, IAnimatorListener
    {
        ...
        public virtual void Stat_ModifyValue(int ID, float modifyvalue) =>
            Stat_Get(ID)?.Modify(modifyvalue);

        public virtual void Stat_ModifyValue(string name, float modifyvalue) =>
            Stat_Get(name)?.Modify(modifyvalue);
        ...
        public virtual void Stat_ModifyValue(
            string name,
            float modifyvalue,
            StatOption modifyType
        ) => Stat_Get(name)?.Modify(modifyvalue, modifyType);
        ...
        public virtual void Stat_Pin_ModifyValue(
            float newValue,
            int ticks,
            ...
            if (staminaID != null)
            {
                ...
                if (method != null)
                {
                    ...
                    UnityEditor.Events.UnityEventTools.AddBoolPersistentListener(
                        staminaStat.OnStatBelow,
                        method,
                        false
                    );
                    UnityEditor.Events.UnityEventTools.AddBoolPersistentListener(
                        staminaStat.OnStatAbove,
                        method,
                        true
                    );
                }
                ...
                if (UIStamina)
                {
                    UnityEditor.Events.UnityEventTools.AddPersistentListener(
                        staminaStat.OnValueChangeNormalized,
                        UIStamina.Invoke
                    );
                    UnityEditor.Events.UnityEventTools.AddPersistentListener(
                        staminaStat.OnStatFull,
                        UIStamina.Invoke
                    );
                }
                ...
                {
                    UnityEditor.Events.UnityEventTools.AddObjectPersistentListener<StatID>(
                        onSprintEnable,
                        Stat_Pin,
                        staminaID
                    );
        ...
        void CreateHealth()
        {
            ...
            if (health != null)
            {
                ...
                if (method != null)
                    UnityEditor.Events.UnityEventTools.AddObjectPersistentListener<StateID>(
                        HealthStat.OnStatEmpty,
                        method,
                        deathID
                    );
                ...
                {
                    UnityEditor.Events.UnityEventTools.AddPersistentListener(
                        HealthStat.OnValueChangeNormalized,
                        UIHealth.Invoke
                    );
                    UnityEditor.Events.UnityEventTools.AddPersistentListener(
                        HealthStat.OnStatFull,
                        UIHealth.Invoke
                    );
                }
    ...
    public class Stat
    {
        ...
        [SerializeField]
        internal FloatReference multiplier = new FloatReference(1);

        /// <summary>Can the Stat regenerate overtime</summary>
        [SerializeField]
        internal BoolReference regenerate = new BoolReference(false);
        ...
        [SerializeField]
        internal BoolReference degenerate = new BoolReference(false);
        ...
        internal void InitializeStat(Stats holder)
        {
            ...
            if (value.Value >= Above)
                isAbove = true; //This means that The Stat Value is over the Above value
            else if (value.Value <= Below)
                isBelow = true; //This means that The Stat Value is under the Below value
        ...
        internal void SetValue(float value)
        {
            ...
            if (
                (!Active)
                || //If the  Stat is not Active do nothing
                (this.value.Value == RealValue)
            )
                return; //If the values are equal do nothing. Avoid Stack Overflow
        ...
        public virtual void Modify(float newValue, int ticks, float timeBetweenTicks)
        {
            ...
            Owner.StartCoroutine(
                out I_ModifyPerTicks,
                C_ModifyTicksValue(newValue, ticks, timeBetweenTicks)
            );
        }
