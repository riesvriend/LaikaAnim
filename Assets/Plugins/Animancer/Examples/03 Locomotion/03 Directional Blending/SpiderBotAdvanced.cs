...
{
    /// <summary>
    /// A <see cref="SpiderBot"/> with a <see cref="MixerTransition2D"/> and <see cref="Rigidbody"/> to allow the
    ...
    public sealed class SpiderBotAdvanced : SpiderBot
    {
        ...
        protected override bool IsMoving => _MovementDirection != default;
        ...
        protected override void Awake()
        {
            ...
            // Workaround ries: walk does not normally loop
            _Move.Events.OnEnd = () => _Move.State.NormalizedTime = 0; // Debug.Log("_Move.Events.OnEnd");
        }
        ...
        {
            if (_Move?.State.IsActive == true)
            {
                //// Clear out the root motion in the animation to prevent flipping back at restart of the clip loop
                //var rootMotionTransform = transform.GetChild(0).Find("RootMotion").Find("Tatra_ROOTSHJnt");
                //rootMotionTransform.position = new Vector3(0, rootMotionTransform.position.y, 0);
            }
        }
