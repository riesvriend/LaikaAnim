    ...
    internal class Telemetry : MonoBehaviour
    {
        ...
        internal static TelemetryLogLevel LogLevel
        {
            get
            {
                ...
                {
                    var telemetryLevelString = EditorPrefs.GetString(
                        Telemetry.TELEMETRY_LOGGING_LEVEL
                    );
                    Enum.TryParse(
                        telemetryLevelString,
                        true,
        ...
        /// <returns>Instance key identifying this specific occurrence of the event.</returns>
        internal static void LogInstantEvent(
        ...
        internal static void AnnotateEvent(int instanceKey, AnnotationKey key, string value)
        {
        ...
        private class TelemetryChannel
        {
            ...
            /// </summary>
            private Dictionary<int, TelemetryEventId> _instanceKeyMap =
                new Dictionary<int, TelemetryEventId>();
            ...
            /// <param name="annotations">Optional annotations to add to the event.</param>
            /// <returns>Instance key identifying this specific occurrence of the event.</returns>
            internal virtual void LogInstantEvent(
                TelemetryEventId eventId,
                Dictionary<AnnotationKey, string> annotations = null
            )
            {
                ...
                var timeStamp = "0"; // DateTimeUtility.ElapsedMilliseconds.ToString();
            ...
            /// <param name="key">The annotation key.</param>
            /// <param name="value">The annotation value.</param>
            internal virtual void AnnotateEvent(int instanceKey, AnnotationKey key, string value)
            {
                if (!this._instanceKeyMap.ContainsKey(instanceKey))
                {
                    LogWarning(
                        $"Attempted to end an event that's invalid or not started. Instance ID: {instanceKey}"
                    );
                    return;
                }
            ...
            internal virtual void EndEvent(int instanceKey, ResultType result)
            {
                ...
                {
                    LogWarning(
                        $"Attempted to end an event that's not started or invalid. Instance ID: {instanceKey}"
                    );
                    return;
                }
                ...
                try
                {
                    foreach (var instanceKey in _instanceKeyMap.Keys)
                    {
                        AnnotateEvent(
                            instanceKey,
                            AnnotationKey.Error,
                            "Telemetry event not ended gracefully"
                        );
            ...
            private static extern bool QplMarkerStart(
                int markerId,
                int instanceKey,
                long timestampMs
            );
            ...
            [DllImport(PluginName)]
            private static extern bool QplMarkerEnd(
                int markerId,
                ResultType boolTypeId,
                int instanceKey,
                long timestampMs
            );

            [DllImport(PluginName)]
            private static extern bool QplMarkerPointCached(
                int markerId,
                int nameHandle,
                int instanceKey,
                long timestampMs
            ...
            [DllImport(PluginName)]
            private static extern bool QplMarkerAnnotation(
                int markerId,
                [MarshalAs(UnmanagedType.LPStr)] string annotationKey,
                [MarshalAs(UnmanagedType.LPStr)] string annotationValue,
                int instanceKey
            );

            [DllImport(PluginName)]
            private static extern bool QplCreateMarkerHandle(
            ...
            private static bool QplMarkerEnd(
                int markerId,
                ResultType boolTypeId,
                int instanceKey,
                long timestampMs
            ...
            private static bool QplMarkerPointCached(
                int markerId,
                int nameHandle,
                int instanceKey,
                long timestampMs
            ...
            private static bool QplMarkerAnnotation(
                int markerId,
                [MarshalAs(UnmanagedType.LPStr)] string annotationKey,
                [MarshalAs(UnmanagedType.LPStr)] string annotationValue,
                int instanceKey
            ...
            private static bool QplCreateMarkerHandle(
                [MarshalAs(UnmanagedType.LPStr)] string name,
                out int nameHandle
            )
            {
        ...
        private class LocalTelemetry : TelemetryChannel
        {
            ...
            /// <inheritdoc/>
            internal override void LogInstantEvent(
                TelemetryEventId eventId,
                Dictionary<AnnotationKey, string> annotations = null
            ) { }

            /// <inheritdoc/>
            internal override void AnnotateEvent(
                int instanceKey,
                AnnotationKey key,
                string value
            ) { }
