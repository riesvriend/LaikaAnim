...
namespace Oculus.Interaction.Input
{
    public class FromOVRControllerHandDataSource : DataSource<HandDataAsset>
    {
        ...
        protected override void UpdateData()
        {
            ...
            float pinchAmount = 0f; //OVRControllerUtility.GetPinchAmount(_ovrController);
            float pinchStrength = _pinchCurve.Evaluate(pinchAmount);
            float gripStrength = _pinchCurve.Evaluate(
                OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, _ovrController)
            );

            _handDataAsset.IsFingerHighConfidence[(int)HandFinger.Thumb] = true;
            _handDataAsset.IsFingerPinching[(int)HandFinger.Thumb] =
                pinchStrength >= 1f || gripStrength >= 1f;
            _handDataAsset.FingerPinchStrength[(int)HandFinger.Thumb] = Mathf.Max(
                pinchStrength,
                gripStrength
            );
        ...
        public void InjectTrackingToWorldTransformer(
            ITrackingToWorldTransformer trackingToWorldTransformer
        )
        {
